...
def get_system_prompt_for_code_tool(add_tool_description: bool = True) -> str:
    system_message = f"""You are a helpful assistant. You can call functions to assist with the user query. Important: You must call only one function at a time. After each function call, wait for the execution result before making the next function call if needed."""
    if add_tool_description:
        system_message += f"""
# Tools
...
You are provided with function signatures within <tools></tools> XML tags:
<tools>
{json.dumps(get_tool_description(), ensure_ascii=False)}
</tools>
